if (ai_turn==false) // if its players turn
{
	...
	{
		if (instance_position(mouse_x,mouse_y,objMunitions))
		{
			// if clicked and mouse is over card, run this script
			scr_get_munitions(); // <-- make sure correct card is selected!
		}
	}
	if (mouse_check_button(mb_left)) // makes seleceted card follow mouse
	{
		with (munitions_selected)
		{
			if (chosen==false)
			{
				x=mouse_x;
	...
	{
		if (munitions_selected!=noone)
		{
			scr_choose_munitions();
			with (objMunitions) {depth=-start_y}
