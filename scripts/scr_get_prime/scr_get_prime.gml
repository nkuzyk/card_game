...
for (i=0;i<size;i+=1) {card[i]=ds_list_find_value(under_mouse,i);} // loop through list
	if (size==3)
	{
		...
		if (get_top==card[0].depth) {prime_selected=card[0];} // checks to see which has the lowest depth
		else if (get_top==card[1].depth) {prime_selected=card[1];} // and sets selected
		else if (get_top==card[2].depth) {prime_selected=card[2];}
	}
	else if (size==2)
	{
		var get_top=min(card[0].depth,card[1].depth);
		if (get_top==card[0].depth) {prime_selected=card[0];}
		else if (get_top==card[1].depth) {prime_selected=card[1];}
	}
	else
	{
		prime_selected=instance_position(mouse_x,mouse_y,objPrime); // if only clicked one
	}
